-- Lightweight, unpaid timed claim: "I'm heading to this spot" so it doesn't
-- get taken while the driver is en route. No listings, no payment — just a
-- short-lived hold layered on top of the existing free/public spots.
...
create index spot_claims_spot_id_idx on spot_claims(spot_id, created_at desc);

alter table spot_claims enable row level security;

create policy "claims are publicly readable" on spot_claims
  for select using (true);
create policy "authenticated users can claim a spot" on spot_claims
  for insert to authenticated with check (user_id = auth.uid());

-- Extend nearby_parking to also return each spot's most recent claim
...
as $$
  select
    ...
    (
      select json_build_object('expires_at', c.expires_at)
      from spot_claims c
