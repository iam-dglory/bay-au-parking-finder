...
-- Fully server-side sensor sync: fetches Melbourne's live sensor feed via
-- pg_net (no client, no Bash, no permission prompt needed), matches to the
-- nearest real spot, and upserts into spot_sensor_status. Scheduled via
-- pg_cron below so it runs reliably regardless of whether any client app is
-- open -- replacing the local Claude Code scheduled task, which depended on
-- the desktop app being open and got stuck waiting on an unattended
-- permission prompt.
