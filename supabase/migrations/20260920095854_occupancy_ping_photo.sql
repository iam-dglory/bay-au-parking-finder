-- Optional photo proof for an occupancy report: "Mark occupied" can now attach a
-- photo of the actual occupied spot, since a single anonymous tap with no evidence
-- is easy to fake. "Mark free" stays a single tap -- claiming a spot is now vacant
-- is much lower-stakes to get wrong than falsely claiming it's occupied.
alter table spot_status_pings add column photo_url text;
