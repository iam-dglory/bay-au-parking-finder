...
create or replace function nearby_parking(lat double precision, lng double precision, radius_m integer default 1000)
returns table (
  id uuid,
  address_text text,
  suburb text,
...
as $$
  select
    ...
    s.lat,
    s.lng,
    extensions.ST_Distance(s.location, extensions.ST_SetSRID(extensions.ST_MakePoint(lng, lat), 4326)::extensions.geography) as distance_m,
    s.created_by,
    coalesce(
