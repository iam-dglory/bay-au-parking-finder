...
export function LocationPicker({ onPick, onUseGps }: { onPick: (lat: number, lng: number, label: string) => void; onUseGps: () => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 overflow-y-auto p-6 text-center">
      ...
      <button onClick={onUseGps} className="w-full max-w-xs shrink-0 rounded-xl bg-slate-900 py-3 font-medium text-white hover:bg-slate-800">
        Use my location
      </button>
      <div className="grid w-full max-w-xs shrink-0 grid-cols-2 gap-2">
        {CITIES.map((city) => (
          <button
            key={city.name}
