...
const UNKNOWN: Availability = { state: 'unknown', label: 'No current reading', color: '#64748b' }
...
export function availability(spot: Pick<ParkingSpot, 'sensor_status' | 'latest_ping' | 'kerbside_id'>, now = new Date()): Availability {
  ...
  return { state, label: `${pingUsable ? 'Community reading' : 'Sensor reading'}: ${state}`, color: state === 'occupied' ? '#e11d48' : '#059669' }
}
